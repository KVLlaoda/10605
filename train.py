...
def prune_linear_14(model, train_loader, val_loader, criterion, optimizer, device):
    ...
    while True:
        best_val_accuracy = -1
        threshold = (cur_iter + 10) * 0.01
        print("CURRENT THRESHOLD:", threshold)
        
        if cur_iter == 0:
            state_dict = torch.load(f'final/best_weights.pt', map_location='cpu')
        else:
        ...
        if best_val_accuracy < BEST_BASE_MODEL_ACCURACY - 0.03:
            ...
            for file_path in directory.iterdir():
                if file_path.is_file():
                    ...
                    if match:
                        ...
                        if idx > max_idx:
                            max_idx = idx
                            linear14_best_version = file_path
            
            linear14_best_version = f'weights/model_weights_linear_14_15.pt'
            state_dict = torch.load(f'{linear14_best_version}', map_location='cpu')
        else:
...
def prune_conv2d_7(model, train_loader, val_loader, criterion, optimizer, device):
    ...
    while True:
        ...
        if cur_iter == 0:
            ...
            conv2d_9_final_version = f'weights/model_weights_conv2d_9_25.pt'
...
def prune_conv2d_3(model, train_loader, val_loader, criterion, optimizer, device):
    ...
    while True:
        ...
        if cur_iter == 0:
            ...
            conv2d_7_final_version = f'weights/model_weights_conv2d_7_13.pt'
        ...
        for epoch in range(num_epochs):
            ...
            if val_accuracy > best_val_accuracy:
                ...
                with open(f'weights/log_conv2d_3_{cur_iter}.txt', 'w') as f:
                    f.write(f"{best_val_accuracy}\n")
        ...
        print(f"Final Sparsity: {sparsity*100:.2f}%, Val Acc: {val_accuracy*100:.4f}, Score: {score:.4f}")
        print("-------------------------------------------------\n")
        if best_val_accuracy < BEST_BASE_MODEL_ACCURACY - 0.04:
            print("Stopping retraining as accuracy dropped too low.")
            return
...
def prune_linear_17(model, train_loader, val_loader, criterion, optimizer, device):
    ...
    while True:
        ...
        if cur_iter == 0:
            state_dict = torch.load(f'weights/model_weights_conv2d_3_23.pt', map_location='cpu')
        else:
            state_dict = torch.load(f'weights/model_weights_linear_17_{cur_iter - 1}.pt', map_location='cpu')
        
        log_path = f'weights/log_linear_17_{cur_iter}.txt'
        if os.path.exists(log_path):
            with open(log_path, 'r') as f:
                best_val_accuracy = float(f.readline().strip().split()[-1])
        model.load_state_dict(state_dict)
        ...
        CONV2D_1, CONV2D_3, CONV2D_7, CONV2D_9, LINEAR_14, LINEAR_17 = get_layers(model)
        unfrozen_layers = [CONV2D_1]
        layer_to_freeze = LINEAR_17
        frozen_layers = [LINEAR_14, CONV2D_3, CONV2D_7, CONV2D_9]
        ...
        for epoch in range(num_epochs):
            ...
            if val_accuracy > best_val_accuracy:
                print(f"\nSaving model with val accuracy: {val_accuracy}\n")
                best_val_accuracy = val_accuracy
                torch.save(model.state_dict(), f'weights/model_weights_linear_17_{cur_iter}.pt', _use_new_zipfile_serialization=False)
                with open(f'weights/log_linear_17_{cur_iter}.txt', 'w') as f:
                    f.write(f"{best_val_accuracy}\n")
                
        state_dict = torch.load(f'weights/model_weights_linear_17_{cur_iter}.pt', map_location='cpu')
        model.load_state_dict(state_dict)
        model = model.to(device)
...
def prune_conv2d_1(model, train_loader, val_loader, criterion, optimizer, device):
    ...
    while True:
        ...
        if cur_iter == 0:
            directory = Path("weights")
            pattern = re.compile(r"model_weights_linear_17_(\d+)\.pt")  # capture the index
            max_idx = -1
            linear17_best_version = None
            ...
            for file_path in directory.iterdir():
                if file_path.is_file():
                    ...
                    if match:
                        ...
                        if idx > max_idx:
                            max_idx = idx
                            linear17_best_version = file_path
            
            linear17_best_version = f'weights/model_weights_linear_17_29.pt'
            state_dict = torch.load(f'{linear17_best_version}', map_location='cpu')
        else:
            state_dict = torch.load(f'weights/model_weights_conv2d_1_{cur_iter - 1}.pt', map_location='cpu')
        log_path = f'weights/log_conv2d_1_{cur_iter}.txt'
        if os.path.exists(log_path):
            with open(log_path, 'r') as f:
                best_val_accuracy = float(f.readline().strip().split()[-1])
        model.load_state_dict(state_dict)
        ...
        CONV2D_1, CONV2D_3, CONV2D_7, CONV2D_9, LINEAR_14, LINEAR_17 = get_layers(model)
        unfrozen_layers = []
        layer_to_freeze = CONV2D_1
        frozen_layers = [CONV2D_9, CONV2D_3, CONV2D_7, LINEAR_17, LINEAR_14]
        ...
        for epoch in range(num_epochs):
            ...
            if val_accuracy > best_val_accuracy:
                print(f"\nSaving model with val accuracy: {val_accuracy}\n")
                best_val_accuracy = val_accuracy
                torch.save(model.state_dict(), f'weights/model_weights_conv2d_1_{cur_iter}.pt', _use_new_zipfile_serialization=False)
                with open(f'weights/log_conv2d_1_{cur_iter}.txt', 'w') as f:
                    f.write(f"{best_val_accuracy}\n")
        state_dict = torch.load(f'weights/model_weights_conv2d_1_{cur_iter}.pt', map_location='cpu')
        model.load_state_dict(state_dict)
        model = model.to(device)
...
# state_dict = torch.load(f'final/my_model_weights_1.pt', map_location='cpu')
# model.load_state_dict(state_dict)
# model = model.to(device)
# print_sparsity(model)
# print(compute_sparsity(model))
